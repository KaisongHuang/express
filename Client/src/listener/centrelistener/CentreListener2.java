...
public class CentreListener2 implements MouseListener, ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == ui.getMyButton()) {
			...
			String carID = ui.getTextField_2().getText();
			...
			String yayunyuan = ui.getMyTextField().getText();
		...
		if (!hasUI1 && e.getSource() == ui.getMyButton_3()) {
			...
			if (selectedRow != -1)
				ui1 = new CentreUI2_1(this, data);
			else {
				ui1 = new CentreUI2_1(this, new Vector<Object>());
			}
		...
		if (e.getSource() == ui.getMyButton_1()) {
			...
			if (selectedRow != -1) // 是否存在选中行
			{
				...
				ui.getModel().setValueAt(carID, selectedRow, 4);
				ui.getModel().setValueAt(jianzhuangyuan, selectedRow, 5);
				ui.getModel().setValueAt(yayunyuan, selectedRow, 6);
				ui.getModel().setValueAt(fee, selectedRow, 7);
		...
		if (e.getSource() == ui.getCancle()) {
			...
			delete(ui.getMyTextField());// yayunyuan
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getSource() == ui.getTable()) {
			...
			String carID = (String) ui.getModel().getValueAt(selectedRow, 4);
			ui.getTextField_2().setText(carID);

			String jianzhuangyuan = (String) ui.getModel().getValueAt(selectedRow, 5);
			ui.getTextField_6().setText(jianzhuangyuan);

			String yayunyuan = (String) ui.getModel().getValueAt(selectedRow, 6);
			ui.getMyTextField().setText(yayunyuan);

			double fee = (Double) ui.getModel().getValueAt(selectedRow, 7);
			ui.getTextField_8().setText("" + fee);

			@SuppressWarnings("unchecked")
			ArrayList<String> barcode = (ArrayList<String>) ui.getModel().getValueAt(selectedRow, 8);
